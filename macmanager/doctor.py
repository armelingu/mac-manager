    ...
    checks.append(
        {
            ...
            "score": cyc_score,
            "max": 15,
            "tip": "Every cycle counts — avoid frequent deep discharges."
            if bat.cycle_count > 300
            else "Plenty of life left.",
        }
    )
    ...
    checks.append(
        {
            ...
            "score": disk_score,
            "max": 20,
            "tip": "Run `mm clean` to see cleanup candidates."
            if free_pct < 20
            else "Healthy space.",
        }
    )
    ...
    checks.append(
        {
            ...
            "tip": (
                ...
                if sys.memory_pressure in ("Warn", "Critical")
                else "Memory breathing well."
            ),
        }
    )
    ...
    checks.append(
        {
            ...
            "score": cpu_score,
            "max": 10,
            "tip": "High CPU — check `mm health` to see the offender."
            if sys.cpu_percent > 70
            else "All quiet.",
        }
    )
